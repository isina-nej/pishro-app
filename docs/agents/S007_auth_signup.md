...
|---|---|
| Agent ID | `S007` |
| Design path | `Screen/Auth/Signup` |
| Persian title | ثبت‌نام |
| Deck file | `../desighn/_capture/01-03-auth.dc.html` |
| Own file (ONLY write here) | `lib/features/auth/presentation/screens/signup_screen.dart` |
| Module | `auth` |
| Note from deck | دو تیک را بزنید تا دکمه فعال شود |
...
```
2. Locate the block for `Screen/Auth/Signup` / «ثبت‌نام».
...
### Phase 2 — Scaffold
1. Create `lib/features/auth/presentation/screens/signup_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/auth/presentation/screens/signup_screen.dart
flutter analyze lib/features/auth/presentation/screens/signup_screen.dart
```
...
- ۰۷ · ثبت‌نام
- Screen/Auth/Signup · دو تیک را بزنید تا دکمه فعال شود
- ۹:۴۱
- تأیید شماره موبایل
