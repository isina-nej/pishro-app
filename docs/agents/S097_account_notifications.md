# Agent S097 — Screen/Account/Notifications
...
|---|---|
| Agent ID | `S097` |
| Design path | `Screen/Account/Notifications` |
| Persian title | اعلان‌ها |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/notifications_screen.dart` |
| Module | `account` |
| Note from deck | بدون مقادیر حساس کامل |
...
```
2. Locate the block for `Screen/Account/Notifications` / «اعلان‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/notifications_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/notifications_screen.dart
flutter analyze lib/features/account/presentation/screens/notifications_screen.dart
```
...
- <div style="font-size: 11px; color: #F05C68; margin-top: 4
