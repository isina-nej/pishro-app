# Agent S045 — Screen/Investment/PlanComparison
...
|---|---|
| Agent ID | `S045` |
| Design path | `Screen/Investment/PlanComparison` |
| Persian title | مقایسه طرح‌ها |
| Deck file | `../desighn/_capture/06-06-investment-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/investment/presentation/screens/plan_comparison_screen.dart` |
| Module | `investment` |
| Note from deck | بدون برنده اعلام‌شده |
...
```
2. Locate the block for `Screen/Investment/PlanComparison` / «مقایسه طرح‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/investment/presentation/screens/plan_comparison_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Investment/PlanComparison — «مقایسه طرح‌ها».`
...
### Phase 5 — Verify
```bash
dart format lib/features/investment/presentation/screens/plan_comparison_screen.dart
flutter analyze lib/features/investment/presentation/screens/plan_comparison_screen.dart
```
...
- ۰۵ · مقایسه طرح‌ها
- Screen/Investment/PlanComparison · بدون برنده اعلام‌شده
...
- تومان (برآورد)
- مشاهده روش محاسبه
- این محاسبه صرفاً یک برآورد بر اساس اطلاعات واردشده و پارامترهای فعلی طرح است و تعهد پرداخت ایجاد نمی‌کند.
- ۰۶ · محاسبه‌گر
- Screen/Investment/Calculator · فقط «برآورد»
- اطلاع‌رسانی ریسک
- ریسک بازار
- ارزش دارایی‌ها ممکن است کاهش یابد.
- ریسک نقدشوندگی
- برداشت زودهنگام ممکن است محدود یا مشمول شرایط باشد.
- ریسک قرارداد و عملیاتی
- تأخیر یا تغییر شرایط طبق مفاد قرارداد ممکن است رخ دهد.
