...
|---|---|
| Agent ID | `S001` |
| Design path | `Screen/Auth/Splash` |
| Persian title | شروع |
| Deck file | `../desighn/_capture/01-03-auth.dc.html` |
| Own file (ONLY write here) | `lib/features/auth/presentation/screens/splash_screen.dart` |
| Module | `auth` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Auth/Splash` / «شروع».
...
### Phase 2 — Scaffold
1. Create `lib/features/auth/presentation/screens/splash_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Auth/Splash — «شروع».`
...
### Phase 5 — Verify
```bash
dart format lib/features/auth/presentation/screens/splash_screen.dart
