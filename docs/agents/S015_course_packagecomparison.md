# Agent S015 — Screen/Course/PackageComparison
...
|---|---|
| Agent ID | `S015` |
| Design path | `Screen/Course/PackageComparison` |
| Persian title | مقایسه بسته |
| Deck file | `../desighn/_capture/02-04-courses-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/package_comparison_screen.dart` |
| Module | `courses` |
| Note from deck | روی هر کارت بزنید |
...
```
2. Locate the block for `Screen/Course/PackageComparison` / «مقایسه بسته».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/package_comparison_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Course/PackageComparison — «مقایسه بسته».`
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/package_comparison_screen.dart
flutter analyze lib/features/courses/presentation/screens/package_comparison_screen.dart
```
...
- 04_COURSES
- صفحه ۰۷–۰۸ · پرداخت بسته عادی و VIP — چک‌باکس رضایت تعاملی است
- ۹:۴۱
- تکمیل خرید
...
- ۰۷ · پرداخت بسته عادی
- Screen/Checkout/Course-Regular
- ۹:۴۱
- <div style="di
