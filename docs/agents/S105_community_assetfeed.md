# Agent S105 — Screen/Community/AssetFeed
...
|---|---|
| Agent ID | `S105` |
| Design path | `Screen/Community/AssetFeed` |
| Persian title | فید دارایی در جامعه |
| Deck file | `../desighn/_capture/08-07-market.dc.html` |
| Own file (ONLY write here) | `lib/features/community/presentation/screens/asset_feed_screen.dart` |
| Module | `community` |
| Note from deck | nested under market |
...
```
2. Locate the block for `Screen/Community/AssetFeed` / «فید دارایی در جامعه».
...
### Phase 2 — Scaffold
1. Create `lib/features/community/presentation/screens/asset_feed_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/community/presentation/screens/asset_feed_screen.dart
flutter analyze lib/features/community/presentation/screens/asset_feed_screen.dart
```
...
- 10px; padding: 11px 13px;">
- تنظیمات/منبع
- Chart → ChartSettings · DataStatus → DataSourceInformation
- ۷۰
- صفحه اصلی تکمیل‌شده
- Auth ۸ + Courses ۲۲ + News ۱۰ + Investment ۱۸ + Market ۱۲ از ۱۲ — کامل
- ۳۴
- صفحه اصلی باقی‌مانده
- ۲ ماژول باقی
- ۱۵
- فریم/Overlay بصری این تحویل
- ۱۲ حالت + ۳ Overlay مصور
- ۱۶
- کامپوننت تازه
- جدا از صفحه و حالت
- تفکیک ماژول‌های باقی‌مانده (صفحه اصلی)
- دوره‌ها/اخبار/سرمایه‌گذاری/بازار — تکمیل ✓
- جامعه تحلیلگران ۱۴
- حساب ۲۰
- جمع باقی‌مانده: ۱۴+۲۰ = ۳۴
- LIGHT
- Light Mode — بازار (07_Market)
- همان صفحات، محتوا، تعاملات و وضعیت‌ها — فقط لایه رنگی Theme/Light.
- 9a
- تحویل ۹ — ماژول بازار (07_Market) · Theme/RoyalGreen
- 07_MARKET
- صفحه ۰۱–۰۴
- ۹:۴۱
- بازار
- زنده · به‌روزرسانی: اکنون
- علاقه‌مندی‌ها
- مشاهده همه
- <div style="flex: 1; b
