# Agent S090 — Screen/Account/Wallet
...
|---|---|
| Agent ID | `S090` |
| Design path | `Screen/Account/Wallet` |
| Persian title | کیف پول |
| Deck file | `../desighn/_capture/10-09-account-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/wallet_screen.dart` |
| Module | `account` |
| Note from deck | بدون برداشت/واریز اختراعی |
...
```
2. Locate the block for `Screen/Account/Wallet` / «کیف پول».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/wallet_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/wallet_screen.dart
flutter analyze lib/features/account/presentation/screens/wallet_screen.dart
```
...
- line-height: 1.85;">این کیف پول برای استفاده در خرید دوره، اشتراک و تأمین وجه سرمایه‌گذاری داخل برنامه است.
- ۰۶ · کیف پول
