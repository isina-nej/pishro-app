# Agent S042 — Screen/Investment/PlanCatalog
...
|---|---|
| Agent ID | `S042` |
| Design path | `Screen/Investment/PlanCatalog` |
| Persian title | فهرست طرح‌ها |
| Deck file | `../desighn/_capture/06-06-investment-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/investment/presentation/screens/plan_catalog_screen.dart` |
| Module | `investment` |
| Note from deck | ریسک با آیکون+متن |
...
```
2. Locate the block for `Screen/Investment/PlanCatalog` / «فهرست طرح‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/investment/presentation/screens/plan_catalog_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/investment/presentation/screens/plan_catalog_screen.dart
flutter analyze lib/features/investment/presentation/screens/plan_catalog_screen.dart
```
...
- div style="font-size: 12px; color: #D9DEE3;">مقایسه هر دو طرح
- ۰۲ · فهرست طرح‌ها
- Screen/Investment/PlanCatalog · ریسک با آیکون+متن
- طرح دریافت ماهیانه ۸٪
- نرخ اعلام‌شده طرح
- ۸٪
- ماهیانه
- طبق شرایط و نحوه محاسبه قرارداد
- این نرخ به‌تنهایی به معنای تضمین پرداخت یا نبود ریسک نیست.
- سطح ریسک: متوسط — مشاهده جزئیات ریسک
- مدت طرح
- مقدار نمونه
- حداقل / حداکثر مبلغ
- ۱۰٬۰۰۰٬۰۰۰ تا مقدار نمونه
- شرایط برداشت
- طبق شرایط قرارداد طرح
- احراز هویت موردنیاز
- سطح پایه
- محاسبه‌گر
- مقایسه طرح‌ها
