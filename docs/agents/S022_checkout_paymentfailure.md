# Agent S022 — Screen/Checkout/PaymentFailure
...
|---|---|
| Agent ID | `S022` |
| Design path | `Screen/Checkout/PaymentFailure` |
| Persian title | پرداخت ناموفق |
| Deck file | `../desighn/_capture/03-04-courses-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/payment_failure_screen.dart` |
| Module | `courses` |
| Note from deck | متمایز از حالت نامشخص |
...
```
2. Locate the block for `Screen/Checkout/PaymentFailure` / «پرداخت ناموفق».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/payment_failure_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/payment_failure_screen.dart
flutter analyze lib/features/courses/presentation/screens/payment_failure_screen.dart
```
