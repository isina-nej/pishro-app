...
|---|---|
| Agent ID | `S008` |
| Design path | `Screen/Auth/OTP` |
| Persian title | کد تأیید |
| Deck file | `../desighn/_capture/01-03-auth.dc.html` |
| Own file (ONLY write here) | `lib/features/auth/presentation/screens/o_t_p_screen.dart` |
| Module | `auth` |
| Note from deck | شمارش معکوس زنده |
...
```
2. Locate the block for `Screen/Auth/OTP` / «کد تأیید».
...
### Phase 2 — Scaffold
1. Create `lib/features/auth/presentation/screens/o_t_p_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/auth/presentation/screens/o_t_p_screen.dart
flutter analyze lib/features/auth/presentation/screens/o_t_p_screen.dart
```
...
- style="height: 20px; display: grid; place-items: center;">
- ۰۸ · کد تأیید
- Screen/Auth/OTP · شمارش معکوس زنده
- 10_SHARED_STATES
- فریم‌های حالت — شمرده نمی‌شوند به‌عنوان صفحه اصلی محصول
- ۹:۴۱
- ورود
- خوش برگشتید
...
- ••••••
- ✕
- رمز عبور واردشده نادرست است.
- فراموشی رمز عبور
- ورود
- حالت خطای اعتبارسنجی
- Screen/Auth/Login · Error
- ۹:۴۱
- <rect x="4.5" y="6" width="3"
