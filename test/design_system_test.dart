...
void main() {
  testWidgets('button renders and fires; disabled and loading do not', (
    ...
    await tester.pumpWidget(
      host(PishroButton(label: 'خرید دوره', onPressed: () => taps++)),
    );
  ...
  testWidgets('phone field normalises Persian digits to ASCII', (tester) async {
    final controller = TextEditingController();
    await tester.pumpWidget(
      host(PishroTextField.phone(controller: controller)),
    );
  ...
  testWidgets(
    'bottom nav has exactly the five deck destinations, no home tab',
    (tester) async {
      var index = 0;
      await tester.pumpWidget(
        host(
      ...
      expect(
        find.text('۳'),
        findsOneWidget,
        reason: 'badge uses Persian digits',
      );
