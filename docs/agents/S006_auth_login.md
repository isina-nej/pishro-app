...
|---|---|
| Agent ID | `S006` |
| Design path | `Screen/Auth/Login` |
| Persian title | ورود |
| Deck file | `../desighn/_capture/01-03-auth.dc.html` |
| Own file (ONLY write here) | `lib/features/auth/presentation/screens/login_screen.dart` |
| Module | `auth` |
| Note from deck | اثر انگشت = واریانت پیکربندی‌شده |
...
```
2. Locate the block for `Screen/Auth/Login` / «ورود».
...
### Phase 2 — Scaffold
1. Create `lib/features/auth/presentation/screens/login_screen.dart` (and directory if needed).
...
- div style="height: 22px; display: grid; place-items: center;">
- ۰۶ · ورود
- Screen/Auth/Login · اثر انگشت = واریانت پیکربندی‌شده
- ۹:۴۱
- ساخت حساب
- نام
- سارا
