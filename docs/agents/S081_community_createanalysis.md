# Agent S081 — Screen/Community/CreateAnalysis
...
|---|---|
| Agent ID | `S081` |
| Design path | `Screen/Community/CreateAnalysis` |
| Persian title | ایجاد تحلیل |
| Deck file | `../desighn/_capture/09-08-community.dc.html` |
| Own file (ONLY write here) | `lib/features/community/presentation/screens/create_analysis_screen.dart` |
| Module | `community` |
| Note from deck | بدون پیش‌انتخاب افشا |
...
```
2. Locate the block for `Screen/Community/CreateAnalysis` / «ایجاد تحلیل».
...
### Phase 2 — Scaffold
1. Create `lib/features/community/presentation/screens/create_analysis_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/community/presentation/screens/create_analysis_screen.dart
flutter analyze lib/features/community/presentation/screens/create_analysis_screen.dart
```
...
- در حال نوشتن…
- متن تحلیل
- متن اصلی تحلیل در این بخش نوشته می‌شود…
- سناریوها
- شرط بی‌اعتبارشدن
- منابع داده
- ۱۲۰ نویسه
...
- بازگشت و ویرایش
- <div style="flex: 1; background: #0F5132; color: #FFFFFF; font-size: 13px; font-weight: 700; padding: 13px; border-radiu
