# Agent S104 — Screen/Account/LegalDocuments
...
|---|---|
| Agent ID | `S104` |
| Design path | `Screen/Account/LegalDocuments` |
| Persian title | اسناد و قوانین |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/legal_documents_screen.dart` |
| Module | `account` |
| Note from deck | بدون تأیید خودکار |
...
```
2. Locate the block for `Screen/Account/LegalDocuments` / «اسناد و قوانین».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/legal_documents_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/LegalDocuments — «اسناد و قوانین».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/legal_documents_screen.dart
flutter analyze lib/features/account/presentation/screens/legal_documents_screen.dart
```
...
- نسخه ۱٫۰
- ۲۰ · اسناد و قوانین
- Screen/Account/LegalDocuments · بدون تأیید خودکار
- 09_ACCOUNT
- Overlay مصور — امنیت، خروج، حریم خصوصی
- خروج از حساب
- خروج از این دستگاه
- خروج از همه دستگاه‌ها
- نشست محلی شما غیرفعال می‌شود. محتوای دانلودشده طبق سیاست برنامه مدیریت می‌شود.
- خروج
- انصراف
- تأیید خروج
- Overlay/Account/LogoutConfirmation · «همه دستگاه‌ها» پیش‌فرض نیست
- تغییر رمز عبور
- رمز عبور فعلی
- رمز عبور جدید
- تکرار رمز عبور جدید
...
- ذخیره رمز جدید
- تغییر رمز عبور
- Overlay/Account/ChangePassword · بدون پیش‌پرشدگی
- درخواست حذف حساب
- برای تأیید، عبارت «حذف حساب» را تایپ کنید
- ثبت درخواست حذف
- تأیید حذف حساب
- Overlay/Account/AccountDeletionConfirmation · اقدام آگاهانه
- 10_SHARED_STATES
- فریم حالت بصری منتخب
