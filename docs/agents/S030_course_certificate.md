# Agent S030 — Screen/Course/Certificate
...
|---|---|
| Agent ID | `S030` |
| Design path | `Screen/Course/Certificate` |
| Persian title | گواهی |
| Deck file | `../desighn/_capture/04-04-courses-part-3.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/certificate_screen.dart` |
| Module | `courses` |
| Note from deck | قاب سبز، تزئین طلایی کم‌مصرف |
...
```
2. Locate the block for `Screen/Course/Certificate` / «گواهی».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/certificate_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/certificate_screen.dart
flutter analyze lib/features/courses/presentation/screens/certificate_screen.dart
```
...
- cy="6" r="2">
- ۰۵ · گواهی
- Screen/Course/Certificate · قاب سبز، تزئین طلایی کم‌مصرف
- 10_SHARED_STATES
- ۱۲ فریم بصری حالت جانبی — شمرده نمی‌شوند به‌عنوان صفحه اصلی محصول
- این جلسه قفل است
- پس از تکمیل جلسات قبلی فصل، این جلسه باز می‌شود.
- ادامه از جلسه فعلی
- جلسه قفل‌شده
- Course/LessonDetails-Locked
- تیم آموزشی پیشرو سرمایه
- VIP
- هنوز پیامی ردوبدل نشده است.
- سؤال خود را درباره این دوره برای مدرس ارسال کنید.
- گفت‌وگو — بدون پیام
- Course/VIPInstructorChat-Empty
- هیچ جلسه‌ای دانلود نشده
- برای دسترسی آفلاین، جلسات را از این صفحه دانلود کنید.
- دانلودها — خالی
- Course/DownloadsAndResources-Empty
- در حال آماده‌سازی گواهی
- چند لحظه صبر کنید…
- تولید گواهی
- Course/Certificate-Generating
