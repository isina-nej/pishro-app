# Agent S036 — Screen/News/SearchResults
...
|---|---|
| Agent ID | `S036` |
| Design path | `Screen/News/SearchResults` |
| Persian title | نتایج جستجو |
| Deck file | `../desighn/_capture/05-05-news.dc.html` |
| Own file (ONLY write here) | `lib/features/news/presentation/screens/search_results_screen.dart` |
| Module | `news` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/News/SearchResults` / «نتایج جستجو».
...
### Phase 2 — Scaffold
1. Create `lib/features/news/presentation/screens/search_results_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/news/presentation/screens/search_results_screen.dart
flutter analyze lib/features/news/presentation/screens/search_results_screen.dart
```
...
- ont-size: 11px; color: #8D97A2;">در حال بارگذاری موارد بیشتر…
- ۰۶ · نتایج جستجو
- Screen/News/SearchResults
- اقتصاد ایران
- نرخ تورم نقطه‌به‌نقطه در گزارش جدید بانک مرکزی اعلام شد
- تحریریه پیشرو سرمایه
- ·
- ۶ مرداد ۱۴۰۵
- ·
- ۴ دقیقه مطالعه
...
- کاربر نمونه ۱
- ۲ ساعت پیش
- تحلیل خوبی بود، ممنون از تیم تحریریه.
- پاسخ‌ها (۲)
- گزارش
- کاربر نمونه ۲
- در انتظار بررسی
- آیا داده‌های این گزارش رسمی است؟
- پاسخ‌ها (۰)
- گزارش
- ثبت دیدگاه
- ۰۸ · دیدگاه‌ها
- Screen/News/Comments
- <div style="width: 410px; padding: 10px; background: #1D2126; border-rad
