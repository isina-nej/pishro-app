# Agent S103 — Screen/Account/Preferences
...
|---|---|
| Agent ID | `S103` |
| Design path | `Screen/Account/Preferences` |
| Persian title | تنظیمات |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/preferences_screen.dart` |
| Module | `account` |
| Note from deck | بدون تغییر پالت RoyalGreen |
...
```
2. Locate the block for `Screen/Account/Preferences` / «تنظیمات».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/preferences_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/preferences_screen.dart
flutter analyze lib/features/account/presentation/screens/preferences_screen.dart
```
...
- iv style="font-size: 11px; color: #F05C68; text-align: center; margin-top: 8px;">بازنشانی تنظیمات به حالت پیش‌فرض
- ۱۹ · تنظیمات
