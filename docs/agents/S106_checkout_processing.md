# Agent S106 — Screen/Checkout/Processing
...
|---|---|
| Agent ID | `S106` |
| Design path | `Screen/Checkout/Processing` |
| Persian title | پرداخت — پردازش (شبکه) |
| Deck file | `../desighn/_capture/02-04-courses-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/processing_screen.dart` |
| Module | `courses` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Checkout/Processing` / «پرداخت — پردازش (شبکه)».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/processing_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/processing_screen.dart
flutter analyze lib/features/courses/presentation/screens/processing_screen.dart
```
