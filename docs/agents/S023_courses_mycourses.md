# Agent S023 — Screen/Courses/MyCourses
...
|---|---|
| Agent ID | `S023` |
| Design path | `Screen/Courses/MyCourses` |
| Persian title | دوره‌های من |
| Deck file | `../desighn/_capture/03-04-courses-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/my_courses_screen.dart` |
| Module | `courses` |
| Note from deck | تب‌ها را بزنید — گفت‌وگوی مدرس فقط روی دوره VIP |
...
```
2. Locate the block for `Screen/Courses/MyCourses` / «دوره‌های من».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/my_courses_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Courses/MyCourses — «دوره‌های من».`
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/my_courses_screen.dart
flutter analyze lib/features/courses/presentation/screens/my_courses_screen.dart
```
...
- ۰۶ · دوره‌های من
- Screen/Courses/MyCourses · تب‌ها را بزنید — گفت‌وگوی مدرس فقط روی دوره VIP
- 04_COURSES
- صفحه ۰۷–۰۸ · داشبورد یادگیری و سرفصل‌های دوره
- ۹:۴۱
- تحلیل تکنیکال از صفر تا معامله‌گری
- ۴۲٪
- ۵ از ۱۲ جلسه تکمیل‌شده
- زمان صرف‌شده: ۳ ساعت و ۲۰ دقیقه
- ادامه یادگیری — جلسه ۵
- ۳ جلسه برای مشاهده آفلاین دانلود شده است.
- سرفصل‌های دوره
- منابع و فایل‌های دوره
- گفت‌وگو با مدرس
- VIP
- ۰۷ · داشبورد یادگیری
- Screen/Course/LearningDashboard · گفت‌وگو با مدرس فقط برای این کاربر VIP قابل مشاهده است
- <div style="font-family: Inter, sans-serif; font-siz
