# Agent S099 — Screen/Account/Devices
...
|---|---|
| Agent ID | `S099` |
| Design path | `Screen/Account/Devices` |
| Persian title | دستگاه‌ها و نشست‌ها |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/devices_screen.dart` |
| Module | `account` |
| Note from deck | IP ماسک، موقعیت تقریبی |
...
```
2. Locate the block for `Screen/Account/Devices` / «دستگاه‌ها و نشست‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/devices_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/Devices — «دستگاه‌ها و نشست‌ها».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/devices_screen.dart
flutter analyze lib/features/account/presentation/screens/devices_screen.dart
```
