...
|---|---|
| Agent ID | `S040` |
| Design path | `Screen/News/Saved` |
| Persian title | ذخیره‌شده‌ها |
| Deck file | `../desighn/_capture/05-05-news.dc.html` |
| Own file (ONLY write here) | `lib/features/news/presentation/screens/saved_screen.dart` |
| Module | `news` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/News/Saved` / «ذخیره‌شده‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/news/presentation/screens/saved_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/news/presentation/screens/saved_screen.dart
flutter analyze lib/features/news/presentation/screens/saved_screen.dart
```
...
- .3v-15a1.5 1.5 0 0 1 1.5-1.5h7A1.5 1.5 0 0 1 17 4.5z">
- ۱۰ · ذخیره‌شده‌ها
- Screen/News/Saved
- 05_NEWS
...
- تبلیغات نامرتبط
- محتوای نامناسب
- <div style="display: flex; align-items: center; gap: 10px; backgroun
