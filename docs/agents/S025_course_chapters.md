# Agent S025 — Screen/Course/Chapters
...
|---|---|
| Agent ID | `S025` |
| Design path | `Screen/Course/Chapters` |
| Persian title | سرفصل‌های دوره |
| Deck file | `../desighn/_capture/03-04-courses-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/chapters_screen.dart` |
| Module | `courses` |
| Note from deck | ۷ حالت درس با آیکون و برچسب |
...
```
2. Locate the block for `Screen/Course/Chapters` / «سرفصل‌های دوره».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/chapters_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/chapters_screen.dart
flutter analyze lib/features/courses/presentation/screens/chapters_screen.dart
```
...
- tyle="font-size: 11px; color: #3B9EFF; flex: none;">پیش‌نمایش
- ۰۸ · سرفصل‌های دوره
- Screen/Course/Chapters · ۷ حالت درس با آیکون و برچسب
- 10_SHARED_STATES
- ۶ فریم حالت جانبی — شمرده نمی‌شوند به‌عنوان صفحه اصلی محصول
...
- وضعیت پرداخت نامشخص
- Checkout/PaymentUnknown · جدا از ناموفق
- روش پرداخت
- بارگذاری روش‌های پرداخت
- Checkout/PaymentMethod-Loading
...
- کد تخفیف نامعتبر
- Checkout/DiscountAndCoin-Invalid
- دوره‌های من
- در حال یادگیری
- تکمیل‌شده
- همه
