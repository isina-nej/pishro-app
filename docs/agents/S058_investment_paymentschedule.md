# Agent S058 — Screen/Investment/PaymentSchedule
...
|---|---|
| Agent ID | `S058` |
| Design path | `Screen/Investment/PaymentSchedule` |
| Persian title | برنامه پرداخت |
| Deck file | `../desighn/_capture/07-06-investment-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/investment/presentation/screens/payment_schedule_screen.dart` |
| Module | `investment` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Investment/PaymentSchedule` / «برنامه پرداخت».
...
### Phase 2 — Scaffold
1. Create `lib/features/investment/presentation/screens/payment_schedule_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/investment/presentation/screens/payment_schedule_screen.dart
flutter analyze lib/features/investment/presentation/screens/payment_schedule_screen.dart
```
...
- r: #D9DEE3;">۴٬۰۰۰٬۰۰۰ تومان
- برنامه‌ریزی‌شده
- ۱۰ · برنامه پرداخت
- Screen/Investment/PaymentSchedule
- 06_INVESTMENT
- Overlay مصور
- تأیید نهایی ثبت درخواست
- طرح
- دریافت ماهیانه ۸٪
- مبلغ
- ۵۰٬۰۰۰٬۰۰۰ تومان
...
- نسخه قرارداد
- ۱٫۲
- با تأیید، درخواست شما برای بررسی و ثبت ارسال می‌شود.
- تأیید و ثبت
- انصراف
- تأیید نهایی
- Overlay/Investment/SubmissionConfirmation
- وضعیت «در انتظار تأمین وجه» یعنی چه؟
- پرداخت شما دریافت شده اما هنوز از سوی درگاه بانکی تأیید نهایی نشده است. این وضعیت خودکار به‌روزرسانی می‌شود.
- تماس با پشتیبانی
- توضیح وضعیت
- Overlay/Investment/StatusExplanation
- جزئیات پرداخت
...
- PAY-1043
- جزئیات پرداخت
- Overlay/Investment/PaymentDetails
- 10_SHARED_STATES
- ۱۰ فریم حالت بصری — شمرده نمی‌شوند به‌عنوان صفحه اصلی محصول
- <svg width="44" height="44" viewBox="0
