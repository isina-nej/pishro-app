...
|---|---|
| Agent ID | `S059` |
| Design path | `Screen/Market/Home` |
| Persian title | بازار (خانه) |
| Deck file | `../desighn/_capture/08-07-market.dc.html` |
| Own file (ONLY write here) | `lib/features/market/presentation/screens/home_screen.dart` |
| Module | `market` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Market/Home` / «بازار (خانه)».
...
### Phase 2 — Scaffold
1. Create `lib/features/market/presentation/screens/home_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Market/Home — «بازار (خانه)».`
...
### Phase 5 — Verify
```bash
dart format lib/features/market/presentation/screens/home_screen.dart
flutter analyze lib/features/market/presentation/screens/home_screen.dart
```
