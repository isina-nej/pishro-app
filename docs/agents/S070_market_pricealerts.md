# Agent S070 — Screen/Market/PriceAlerts
...
|---|---|
| Agent ID | `S070` |
| Design path | `Screen/Market/PriceAlerts` |
| Persian title | هشدارهای قیمت |
| Deck file | `../desighn/_capture/08-07-market.dc.html` |
| Own file (ONLY write here) | `lib/features/market/presentation/screens/price_alerts_screen.dart` |
| Module | `market` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Market/PriceAlerts` / «هشدارهای قیمت».
...
### Phase 2 — Scaffold
1. Create `lib/features/market/presentation/screens/price_alerts_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/market/presentation/screens/price_alerts_screen.dart
flutter analyze lib/features/market/presentation/screens/price_alerts_screen.dart
```
...
- ۳٬۵۰۰٬۰۰۰٬۰۰۰
- تومان
- قیمت فعلی: ۳٬۴۲۰٬۰۰۰٬۰۰۰ تومان — فقط برای مرجع
- اعلان درون‌برنامه‌ای
- ساخت هشدار
- ساخت هشدار
- Overlay/Market/CreatePriceAlert · بدون پیش‌پرشدگی پیش‌بینانه
- درباره این داده
- منبع داده
- سرویس داده نمونه
- وضعیت
...
- آخرین به‌روزرسانی
- ۲ دقیقه پیش
- قیمت‌ها و آمار نمایش‌داده‌شده نمونه‌اند تا زمانی که به سرویس داده واقعی متصل شوند.
- اطلاعات منبع داده
- Overlay/Market/DataSourceInformation
- تنظیمات نمودار
- نوع نمودار
- خطی
- کندل
