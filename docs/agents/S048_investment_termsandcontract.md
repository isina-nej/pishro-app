# Agent S048 — Screen/Investment/TermsAndContract
...
|---|---|
| Agent ID | `S048` |
| Design path | `Screen/Investment/TermsAndContract` |
| Persian title | شرایط و قرارداد |
| Deck file | `../desighn/_capture/06-06-investment-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/investment/presentation/screens/terms_and_contract_screen.dart` |
| Module | `investment` |
| Note from deck | فقط مرور، بدون امضا |
...
```
2. Locate the block for `Screen/Investment/TermsAndContract` / «شرایط و قرارداد».
...
### Phase 2 — Scaffold
1. Create `lib/features/investment/presentation/screens/terms_and_contract_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Investment/TermsAndContract — «شرایط و قرارداد».`
...
### Phase 5 — Verify
```bash
dart format lib/features/investment/presentation/screens/terms_and_contract_screen.dart
flutter analyze lib/features/investment/presentation/screens/terms_and_contract_screen.dart
```
...
- توضیح سطح ریسک
- Overlay/Investment/RiskLevelExplanation
- روش محاسبه برآورد
- نسخه محاسبه: نمونه · آخرین به‌روزرسانی پارامترها: امروز
- روش محاسبه
- Overlay/Investment/CalculationMethodology
- 10_SHARED_STATES
- ۸ فریم حالت بصری — شمرده نمی‌شوند به‌عنوان صفحه اصلی محصول
- خانه — بارگذاری
- Investment/Home-Loading
- هنوز سرمایه‌گذاری فعالی ندارید
- مشاهده طرح‌ها
- خانه — بدون طرح فعال
- Investment/Home-NoActiveInvestment
