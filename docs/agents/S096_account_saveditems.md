# Agent S096 — Screen/Account/SavedItems
...
|---|---|
| Agent ID | `S096` |
| Design path | `Screen/Account/SavedItems` |
| Persian title | ذخیره‌شده‌ها |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/saved_items_screen.dart` |
| Module | `account` |
| Note from deck | محتوای حذف‌شده مخفی نمی‌شود |
...
```
2. Locate the block for `Screen/Account/SavedItems` / «ذخیره‌شده‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/saved_items_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/SavedItems — «ذخیره‌شده‌ها».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/saved_items_screen.dart
flutter analyze lib/features/account/presentation/screens/saved_items_screen.dart
```
...
- ۲ دستگاه
- توصیه‌های امنیتی
- فعال‌سازی تأیید دومرحله‌ای امنیت حساب را افزایش می‌دهد.
