# Agent S018 — Screen/Checkout/PaymentMethod
...
|---|---|
| Agent ID | `S018` |
| Design path | `Screen/Checkout/PaymentMethod` |
| Persian title | روش پرداخت |
| Deck file | `../desighn/_capture/03-04-courses-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/courses/presentation/screens/payment_method_screen.dart` |
| Module | `courses` |
| Note from deck | روی هر ردیف بزنید |
...
```
2. Locate the block for `Screen/Checkout/PaymentMethod` / «روش پرداخت».
...
### Phase 2 — Scaffold
1. Create `lib/features/courses/presentation/screens/payment_method_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/courses/presentation/screens/payment_method_screen.dart
flutter analyze lib/features/courses/presentation/screens/payment_method_screen.dart
```
