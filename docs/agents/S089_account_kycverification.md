# Agent S089 — Screen/Account/KYCVerification
...
|---|---|
| Agent ID | `S089` |
| Design path | `Screen/Account/KYCVerification` |
| Persian title | احراز هویت — تکمیل |
| Deck file | `../desighn/_capture/10-09-account-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/k_y_c_verification_screen.dart` |
| Module | `account` |
| Note from deck | بدون رضایت پیش‌فرض |
...
```
2. Locate the block for `Screen/Account/KYCVerification` / «احراز هویت — تکمیل».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/k_y_c_verification_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/k_y_c_verification_screen.dart
flutter analyze lib/features/account/presentation/screens/k_y_c_verification_screen.dart
```
...
- ۰۵ · احراز هویت — تکمیل
- Screen/Account/KYCVerification · بدون رضایت پیش‌فرض
- کیف پول
- موجودی قابل استفاده
- ۱۲٬۴۰۰٬۰۰۰
- تومان
...
- به‌روزرسانی: اکنون
- تراکنش‌های کیف پول
- این کیف پول برای استفاده در خرید دوره، اشتراک و تأمین وجه سرمایه‌گذاری داخل برنامه است.
- ۰۶ · کیف پول
- Screen/Account/Wallet · بدون برداشت/واریز اختراعی
- تراکنش‌های کیف پول
- همه
- این ماه
- در انتظار
...
- +۴٬۰۰۰٬۰۰۰
- تکمیل‌شده
- <div style="display: flex; align-items: center; gap: 10px; background: #14181D; border: 1px solid #
