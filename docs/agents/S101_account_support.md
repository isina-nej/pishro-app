# Agent S101 — Screen/Account/Support
...
|---|---|
| Agent ID | `S101` |
| Design path | `Screen/Account/Support` |
| Persian title | پشتیبانی |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/support_screen.dart` |
| Module | `account` |
| Note from deck | بدون ساعت/شماره اختراعی |
...
```
2. Locate the block for `Screen/Account/Support` / «پشتیبانی».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/support_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/Support — «پشتیبانی».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/support_screen.dart
flutter analyze lib/features/account/presentation/screens/support_screen.dart
```
...
- -align: center; min-height: 44px; display: flex; align-items: center; justify-content: center;">ثبت درخواست جدید
- ۱۷ · پشتیبانی
...
- <div style="display: flex; flex-direction: c
