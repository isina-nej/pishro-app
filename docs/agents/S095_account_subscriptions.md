# Agent S095 — Screen/Account/Subscriptions
...
|---|---|
| Agent ID | `S095` |
| Design path | `Screen/Account/Subscriptions` |
| Persian title | اشتراک‌ها |
| Deck file | `../desighn/_capture/11-09-account-part-2.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/subscriptions_screen.dart` |
| Module | `account` |
| Note from deck | پرداخت≠دسترسی |
...
```
2. Locate the block for `Screen/Account/Subscriptions` / «اشتراک‌ها».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/subscriptions_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/Subscriptions — «اشتراک‌ها».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/subscriptions_screen.dart
flutter analyze lib/features/account/presentation/screens/subscriptions_screen.dart
```
...
- yle="font-size: 10px; color: #8D97A2;">پرداخت ثبت شد؛ فعال‌سازی محتوا هنوز تأیید نشده است.
- ۱۱ · اشتراک‌ها
- Screen/Account/Subscriptions · پرداخت≠دسترسی
- ذخیره‌شده‌ها
- همه
- دوره‌ها
- اخبار
...
- دوره · ذخیره: ۳ روز پیش
- در دسترس
- این خبر دیگر در دسترس نیست
- خبر · ذخیره: ۱ هفته پیش
- حذف‌شده
- حذف
- ۱۲ · ذخیره‌شده‌ها
- Screen/Account/SavedItems · محتوای حذف‌شده مخفی نمی‌شود
- علامت‌گذاری همه
- اعلان‌ها
- همه
- امنیت
...
- ورود از دستگاه جدید ثبت شد. اگر شما نبودید، امنیت حساب را بررسی کنید.
- ۲۰ دقیقه پیش · امنیت
- پرداخت اشتراک تأیید شد
- دیروز · پرداخت
- ۱۳ · اعلان‌ها
- Screen/Account/Notifications · بدون مقادیر حساس کامل
- امنیت حساب
