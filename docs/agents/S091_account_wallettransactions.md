# Agent S091 — Screen/Account/WalletTransactions
...
|---|---|
| Agent ID | `S091` |
| Design path | `Screen/Account/WalletTransactions` |
| Persian title | تراکنش‌های کیف پول |
| Deck file | `../desighn/_capture/10-09-account-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/wallet_transactions_screen.dart` |
| Module | `account` |
| Note from deck | نامشخص جدا از ناموفق |
...
```
2. Locate the block for `Screen/Account/WalletTransactions` / «تراکنش‌های کیف پول».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/wallet_transactions_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/wallet_transactions_screen.dart
flutter analyze lib/features/account/presentation/screens/wallet_transactions_screen.dart
```
...
- or: #65707C;">نامشخص
- به‌روزرسانی: اکنون
- ۰۷ · تراکنش‌های کیف پول
- Screen/Account/WalletTransactions · نامشخص جدا از ناموفق
- کوین پیشرو
- موجودی کوین
- غیرقابل برداشت
- ۳٬۲۵۰
- کوین پیشرو یک امتیاز داخلی و غیرقابل برداشت است.
- دریافت‌شده
- ۴٬۱۰۰
- استفاده‌شده
- ۸۵۰
...
- ۲۰۰
- فعالیت اخیر
- تکمیل دوره — دریافت‌شده
- +۱۰۰
- تخفیف خرید — استفاده‌شده
- −۵۰
...
- در انتظار پرداخت
- <div style="font-size: 10px; color: #8D97A2;"
