# Agent S086 — Screen/Account/Profile
...
|---|---|
| Agent ID | `S086` |
| Design path | `Screen/Account/Profile` |
| Persian title | اطلاعات حساب |
| Deck file | `../desighn/_capture/10-09-account-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/profile_screen.dart` |
| Module | `account` |
| Note from deck | فیلد قفل‌شده |
...
```
2. Locate the block for `Screen/Account/Profile` / «اطلاعات حساب».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/profile_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/Profile — «اطلاعات حساب».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/profile_screen.dart
flutter analyze lib/features/account/presentation/screens/profile_screen.dart
```
