# Agent S065 — Screen/Market/Search
...
|---|---|
| Agent ID | `S065` |
| Design path | `Screen/Market/Search` |
| Persian title | جستجوی ارز |
| Deck file | `../desighn/_capture/08-07-market.dc.html` |
| Own file (ONLY write here) | `lib/features/market/presentation/screens/search_screen.dart` |
| Module | `market` |
| Note from deck | فارسی + لاتین |
...
```
2. Locate the block for `Screen/Market/Search` / «جستجوی ارز».
...
### Phase 2 — Scaffold
1. Create `lib/features/market/presentation/screens/search_screen.dart` (and directory if needed).
...
### Phase 5 — Verify
```bash
dart format lib/features/market/presentation/screens/search_screen.dart
flutter analyze lib/features/market/presentation/screens/search_screen.dart
```
...
- Inter, sans-serif; font-size: 10px; color: #8D97A2; margin-inline-start: 6px;">SOL
- ۰۷ · جستجوی ارز
