# Agent S060 — Screen/Market/AllAssets
...
|---|---|
| Agent ID | `S060` |
| Design path | `Screen/Market/AllAssets` |
| Persian title | همه ارزها |
| Deck file | `../desighn/_capture/08-07-market.dc.html` |
| Own file (ONLY write here) | `lib/features/market/presentation/screens/all_assets_screen.dart` |
| Module | `market` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Market/AllAssets` / «همه ارزها».
...
### Phase 2 — Scaffold
1. Create `lib/features/market/presentation/screens/all_assets_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Market/AllAssets — «همه ارزها».`
...
### Phase 5 — Verify
```bash
dart format lib/features/market/presentation/screens/all_assets_screen.dart
flutter analyze lib/features/market/presentation/screens/all_assets_screen.dart
```
...
- ۸٬۹۲۰٬۰۰۰
- +۵٫۱۲٪
- داده‌ها با تأخیر نمایش داده می‌شوند.
- ۰۳ · علاقه‌مندی‌ها
