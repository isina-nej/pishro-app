# Agent S088 — Screen/Account/KYCOverview
...
|---|---|
| Agent ID | `S088` |
| Design path | `Screen/Account/KYCOverview` |
| Persian title | احراز هویت — نمای کلی |
| Deck file | `../desighn/_capture/10-09-account-part-1.dc.html` |
| Own file (ONLY write here) | `lib/features/account/presentation/screens/k_y_c_overview_screen.dart` |
| Module | `account` |
| Note from deck | — |
...
```
2. Locate the block for `Screen/Account/KYCOverview` / «احراز هویت — نمای کلی».
...
### Phase 2 — Scaffold
1. Create `lib/features/account/presentation/screens/k_y_c_overview_screen.dart` (and directory if needed).
2. Header doc comment:
   `/// Screen/Account/KYCOverview — «احراز هویت — نمای کلی».`
...
### Phase 5 — Verify
```bash
dart format lib/features/account/presentation/screens/k_y_c_overview_screen.dart
flutter analyze lib/features/account/presentation/screens/k_y_c_overview_screen.dart
```
...
- اطلاعات من برای بررسی هویت ارسال می‌شود و می‌دانم تکمیل احراز هویت ممکن است فوری نباشد.
- قبلی
- ادامه
- ۰۵ · احراز هویت — تکمیل
